...
#define PREVIEW_GROUP_SPACING       4
...
/* window preview */
static void               xfce_tasklist_preview_hide                     (XfceTasklist         *tasklist);
static void               xfce_tasklist_preview_show                     (XfceTasklist         *tasklist,
                                                                          XfceTasklistChild    *child);
static void               xfce_tasklist_preview_show_group               (XfceTasklist         *tasklist,
...
xfce_tasklist_class_init (XfceTasklistClass *klass)
{
  ...
  g_object_class_install_property (gobject_class,
                                   PROP_SHOW_WINDOW_PREVIEWS,
                                   g_param_spec_boolean ("show-window-previews",
                                                         NULL, NULL,
                                                         FALSE,
                                                         G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
...
static void
xfce_tasklist_scale_factor (XfceTasklist *tasklist)
{
  gint scale_factor = gtk_widget_get_scale_factor (GTK_WIDGET (tasklist));
...
xfce_tasklist_init (XfceTasklist *tasklist)
{
...
#endif
  tasklist->show_window_previews = FALSE;
  ...
  xfce_tasklist_scale_factor (tasklist);
  g_signal_connect (tasklist, "notify::scale-factor", G_CALLBACK (xfce_tasklist_scale_factor), NULL);
}
                               ...
                               int         type)
{
  ...
  if (type == CHILD_TYPE_GROUP_MENU || size < WNCK_DEFAULT_ICON_SIZE)
                                ...
                                gint *height)
{
  switch (tasklist->preview_size)
    ...
    case XFCE_PREVIEW_SIZE_LARGE:
      ...
      *height = 175;
      break;
    case XFCE_PREVIEW_SIZE_DEFAULT:
    default:
      *width = 220;
      *height = 145;
      break;
                                        ...
                                        WnckWindow *window)
{
  ...
  gint icon_w, icon_h, offset_x, offset_y;
  ...
  if (icon != NULL)
    {
      ...
      offset_x = (target_width - icon_w) / 2;
      offset_y = (target_height - icon_h) / 2;
                                       ...
                                       XfceTasklistChild *child)
{
...
#ifdef GDK_WINDOWING_X11
  {
    ...
    /* Get the GdkWindow for the WnckWindow */
    ...
    if (gdkwindow != NULL && !is_minimized)
      {
        ...
        /* Handle edge case: very small windows */
        ...
        if (raw_pixbuf != NULL)
          {
            /* Create uniform size pixbuf with thin border */
            pixbuf = gdk_pixbuf_new (GDK_COLORSPACE_RGB, TRUE, 8, target_width, target_height);
            gdk_pixbuf_fill (pixbuf, 0x303030ff);  /* dark gray background */
                                              ...
                                              GCallback click_callback)
{
  ...
  /* Store references with proper ref counting */
...
/* Create the control bar with window buttons */
static GtkWidget *
                                           ...
                                           WnckWindow *window)
{
  ...
  minimize_button = xfce_tasklist_preview_create_control_button (
      tasklist, window, "window-minimize-symbolic", "Minimize",
  ...
  maximize_button = xfce_tasklist_preview_create_control_button (
      tasklist, window, "window-maximize-symbolic", "Maximize",
                             ...
                             XfceTasklist *tasklist)
{
  GtkAllocation alloc;
  GtkStyleContext *context;
  GdkRGBA bg_color = {0.15, 0.15, 0.15, 0.95};
  ...
  cairo_arc (cr, alloc.width - radius, radius, radius, -G_PI_2, 0);
  cairo_arc (cr, alloc.width - radius, alloc.height - radius, radius, 0, G_PI_2);
  cairo_arc (cr, radius, alloc.height - radius, radius, G_PI_2, G_PI);
...
/* Timeout callback to hide the preview */
static gboolean
...
/* Preview popup mouse enter event */
static gboolean
...
/* Preview popup mouse leave event */
static gboolean
                                     ...
                                     GdkPixbuf *pixbuf)
{
  ...
  gtk_widget_set_margin_start (outer_box, 6);
  gtk_widget_set_margin_end (outer_box, 6);
  ...
  if (window_name != NULL)
    {
      ...
      gtk_widget_set_margin_top (title_box, 6);
      gtk_widget_set_margin_bottom (title_box, 2);

      /* Window icon */
      icon_pixbuf = wnck_window_get_mini_icon (window);
      if (icon_pixbuf != NULL)
        {
          icon_image = gtk_image_new_from_pixbuf (icon_pixbuf);
          gtk_box_pack_start (GTK_BOX (title_box), icon_image, FALSE, FALSE, 0);
        }

      /* Window title */
      title_label = gtk_label_new (window_name);
      ...
      gtk_box_pack_start (GTK_BOX (outer_box), title_box, FALSE, FALSE, 0);
    }
                             ...
                             XfceTasklistChild *child)
{
  ...
  screen = gtk_widget_get_screen (tasklist->preview_window);
  ...
  /* Calculate position */
  gtk_widget_get_allocation (child->button, &button_alloc);
  gdk_window_get_origin (gtk_widget_get_window (child->button), &x, &y);
  x += button_alloc.x + (button_alloc.width / 2) - (target_width + PREVIEW_PADDING * 2) / 2;

  display = gtk_widget_get_display (GTK_WIDGET (tasklist));
  monitor_num = gdk_screen_get_monitor_at_window (screen, gtk_widget_get_window (child->button));
  gdk_screen_get_monitor_geometry (screen, monitor_num, &monitor_geo);

  if (tasklist->mode == XFCE_PANEL_PLUGIN_MODE_HORIZONTAL)
    {
      if (y < monitor_geo.y + monitor_geo.height / 2)
        y += button_alloc.y + button_alloc.height + 4;
      else
        y += button_alloc.y - (target_height + PREVIEW_PADDING * 2) - 4;
    }
                                   ...
                                   XfceTasklistChild *group_child)
{
  ...
  if (!gtk_widget_get_realized (group_child->button))
    return;
  ...
  gtk_container_set_border_width (GTK_CONTAINER (main_box), PREVIEW_PADDING);
  gtk_container_add (GTK_CONTAINER (event_box), main_box);

  /* Iterate over windows in the group */
  for (lp = group_child->windows; lp != NULL; lp = lp->next)
    {
      XfceTasklistChild *window_child = lp->data;
      GdkPixbuf *pixbuf;

      if (window_child == NULL || window_child->window == NULL)
        continue;
      ...
      frame_widget = xfce_tasklist_preview_create_frame (tasklist, window_child->window, pixbuf);
      gtk_box_pack_start (GTK_BOX (main_box), frame_widget, FALSE, FALSE, 0);
      g_object_unref (pixbuf);

      total_width += target_width + PREVIEW_PADDING * 2 + 8;
    }
  ...
  /* Calculate position */
  gtk_widget_get_allocation (group_child->button, &button_alloc);
  gdk_window_get_origin (gtk_widget_get_window (group_child->button), &x, &y);
  x += button_alloc.x + (button_alloc.width / 2) - (total_width) / 2;

  monitor_num = gdk_screen_get_monitor_at_window (screen, gtk_widget_get_window (group_child->button));
  gdk_screen_get_monitor_geometry (screen, monitor_num, &monitor_geo);

  /* Keep within monitor bounds */
  if (x < monitor_geo.x)
    x = monitor_geo.x;
  if (x + total_width > monitor_geo.x + monitor_geo.width)
    x = monitor_geo.x + monitor_geo.width - total_width;
...
/* Enter notify handler for group buttons */
static gboolean
